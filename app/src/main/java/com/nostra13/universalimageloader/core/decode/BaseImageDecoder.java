...
public class BaseImageDecoder implements ImageDecoder {
    ...
    protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
        ...
        if (scaleType != ImageScaleType.NONE) {
            ...
            if (loggingEnabled)
                L.i(LOG_SABSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), scale, decodingInfo.getImageKey());
        }
    ...
    protected Bitmap considerExactScaleAndOrientaiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo, int rotation, boolean flipHorizontal) {
        ...
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            ...
            if (Float.compare(scale, 1f) != 0) {
                ...
                if (loggingEnabled)
                    L.i(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), scale, decodingInfo.getImageKey());
            }
        }
        // Flip bitmap if need
        if (flipHorizontal) {
