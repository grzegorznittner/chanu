...
public class NetworkProfileManager {
    ...
    synchronized public void ensureInitialized(ChanIdentifiedActivity newActivity) {
        if (!initialized) {
            initialized = true;
            if (DEBUG)
                Log.i(TAG, "ensureInitialized not initialized, initializing newActivity=" + newActivity.getChanActivityId());
            ...
            if (DEBUG)
                Log.i(TAG, "ensureInitialized complete newActivity=" + newActivity.getChanActivityId());
...
*/
    ...
    public void updateViewData(ChanIdentifiedActivity newActivity) {
        if (DEBUG)
            Log.i(TAG, "updateViewData " + newActivity.getChanActivityId(), new Exception("updateViewData"));
        if (newActivity == null) {
            return;
        }
        currentActivityId = newActivity.getChanActivityId();
        ...
        public static void checkNetwork(Context context) {
            ...
            if (activeNetwork != null && activeNetwork.isConnected()) {
                if (activeNetwork.getType() == ConnectivityManager.TYPE_WIFI) {
                    ...
                    if (DEBUG)
                        Log.i(TAG, "Connected to other type of network " + activeNetwork.getType());
