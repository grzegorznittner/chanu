...
public abstract class AbstractNetworkProfile implements NetworkProfile {
    ...
    public Health getConnectionHealth() {
        ...
        if (clonedTransfers.size() < 2) {
            ...
            if (currentHealth != defaultHealth) {
                if (DEBUG)
                    Log.i(TAG, "Less than 2 transfers, changing default health to " + defaultHealth);
        ...
        if (failures > 2) {
            if (currentHealth != Health.BAD) {
                if (DEBUG)
                    Log.i(TAG, "More than 2 failures, switching to BAD from " + currentHealth);
    ...
    public void onThreadSelected(Context context, String board, long threadId) {
        if (DEBUG)
            Log.d(TAG, "onThreadSelected called with board: " + board + " threadId: " + threadId);
        usageCounter++;
    }
    ...
    public void onThreadRefreshed(Context context, Handler handler, String board, long threadId) {
        if (DEBUG)
            Log.d(TAG, "onThreadRefreshed called with board: " + board + " threadId: " + threadId);
        usageCounter++;
    }
    ...
    public void onFullImageLoading(Context context, String board, long threadId, long postId) {
        if (DEBUG)
            Log.d(TAG, "onFullImageLoading called with board: " + board + " threadId: " + threadId + " postId: " + postId);
        usageCounter++;
    }
    ...
    public void onDataFetchSuccess(ChanIdentifiedService service, int time, int size) {
        if (DEBUG)
            Log.i(TAG, "finishedFetchingData called for " + service + " " + size + " bytes during " + time + "ms");

        storeDataTransfer(time, size);

        ChanActivityId data = service.getChanActivityId();
        if (DEBUG)
            Log.i(TAG, "fetchData success for /" + data.boardCode + "/" + data.threadNo + "/" + data.postNo + " priority=" + data.priority);
