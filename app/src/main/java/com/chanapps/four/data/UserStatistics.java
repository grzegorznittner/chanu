    ...
    */
    ...
    public boolean convertThreadStats() {
        if (threadStats.size() > 0) {
            ...
            threadStats.clear();
            if (DEBUG)
                Log.i(TAG, "" + threadsToConvert + " thread stats has been converted to new format.");
            return true;
        }
        return false;
    }
    ...
    public ChanFeature nextTipForPage(TutorialOverlay.Page tutorialPage) {
        ...
        */
        if (DEBUG)
            Log.i(TAG, "nextTipForPage tipSet=" + Arrays.toString(tipSet) + " displayedTips=" + Arrays.toString(displayedTips.toArray()));
