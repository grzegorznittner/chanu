...
public class ThreadListener {
    ...
    public View.OnClickListener expandedImageListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (threadViewable == null) return;
            if (threadViewable.getAbsListView() == null) return;
            int pos = -1;
            try {
            ...
            //    postNo = 0;
            if (DEBUG)
                Log.i(TAG, "expandImageListener /" + boardCode + "/" + threadNo + "#p" + postNo);
            if (postNo > 0)
                GalleryViewActivity.startActivity(v.getContext(), boardCode, threadNo, postNo);
            else GalleryViewActivity.startAlbumViewActivity(v.getContext(), boardCode, threadNo);
        }
    };
    public final View.OnClickListener exifOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if ((Boolean) itemView.getTag(R.id.THREAD_VIEW_IS_EXIF_EXPANDED)) return;
            ...
            if ((flags & (ChanPost.FLAG_HAS_EXIF)) > 0) {
                (new ThreadExpandExifOnClickListener(threadViewable.getAbsListView(), cursor, threadViewable.getHandler())).onClick(itemView);
    ...
    public final View.OnClickListener thumbOnClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if (threadViewable == null || threadViewable.getAbsListView() == null) return;
            int pos = threadViewable.getAbsListView().getPositionForView(v);
            if (DEBUG) Log.i(TAG, "received item click pos: " + pos);
            ...
            if (viewHolder.list_item_image_header != null && viewHolder.list_item_image_expanded_wrapper.getVisibility() == View.VISIBLE && (
                ...
                return;
            } else if (viewHolder.list_item_image_header == null && viewHolder.list_item_image_expanded_wrapper.getVisibility() == View.VISIBLE) { // hide and return
            ...
            ThreadImageExpander expander = (new ThreadImageExpander(viewHolder, cursor, true, isDark ? R.drawable.stub_image_background_dark : R.drawable.stub_image_background, expandedImageListener, true));
    ...
    private final SpannableOnClickListener createPopupListener(final ThreadPopupDialogFragment.PopupType popupType) {
        return new SpannableOnClickListener() {
            ...
            private void locatePost(View v) {
                ...
                if (threadViewable == null) return;
                if (threadViewable.getAbsListView() == null) return;
                if (threadViewable.getAdapter() == null) return;
                Cursor cursor = threadViewable.getAdapter().getCursor();
                pos = threadViewable.getAbsListView().getPositionForView(v);
            ...
            private void locatePost(View v, long cursorId) {
                ...
                postNo = cursorId;
                if (DEBUG)
                    Log.i(TAG, "locatePost() looking for postNo=" + postNo + " out of " + cursor.getCount() + " cursor items");
                ...
                while (cursor.moveToNext()) if (cursor.getLong(col) == postNo) {
            ...
            private void launchThread(Activity activity, long threadNo) {
                ...
                if (cursor.moveToFirst()) {
                    boardCode = cursor.getString(cursor.getColumnIndex(ChanPost.POST_BOARD_CODE));
                    if (DEBUG)
                        Log.i(TAG, "locatePost() launching thread activity /" + boardCode + "/" + threadNo);
            ...
            public void onClick(View v, long cursorId) {
                ...
                if (DEBUG)
                    Log.i(TAG, "popupListener clicked pos=" + pos + " type=" + popupType + " popup postNo=" + postNo);
                if (pos >= 0)
